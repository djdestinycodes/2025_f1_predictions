...
# load the 2024 Great Britain (Silverstone) session data
session_2024 = fastf1.get_session(2024, 12, "R")  # Silverstone is usually round 12
session_2024.load()
...
# quali data from Silverstone GP (placeholder, update with real data if available)
...
# get weather data for Silverstone, UK
...
try:
    ...
    forecast_time = "2025-07-06 13:00:00"  # 14:00 local time (BST)
    forecast_data = next((f for f in weather_data["list"] if f["dt_txt"] == forecast_time), None)
    if forecast_data:
        rain_probability = forecast_data["pop"]
        temperature = forecast_data["main"]["temp"]
        print(f"\n🌤️ Weather Forecast for British GP:")
...
if rain_probability >= 0.75:
    qualifying_2025["QualifyingTime"] = qualifying_2025["QualifyingTime (s)"] * qualifying_2025.get("WetPerformanceFactor", 1)
else:
...
# average position change at Silverstone (placeholder values)
average_position_change_gb = {
    "VER": -1.0,  
    "NOR": 1.0,
    "PIA": 0.2,
    "RUS": 0.5,
...
qualifying_2025["AveragePositionChange"] = qualifying_2025["Driver"].map(average_position_change_gb)
...
print("\n🏁 Predicted 2025 British GP Winner 🏁\n")
